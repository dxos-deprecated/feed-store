    ...
    this._stream = from2.obj(async (size, next) => {
      ...
      const tryReading = async () => {
        ...
        for(const feed of this._feeds.values()) {
          ...
          const msg = feed.message ?? feed.stream.read();
          console.log('reading from feed', feed.descriptor.path, msg, feed.stream._readableState.flowing)
          if(!msg) continue;
      ...
      while(!await tryReading()) {
        await new Promise(resolve => { this._wakeUpReader = resolve; });
      }
    });
  }
